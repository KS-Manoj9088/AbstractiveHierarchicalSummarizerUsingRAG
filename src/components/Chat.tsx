...
export default function Chat({
    ...
    for (const file of Array.from(files)) {
      if (file.type === 'application/pdf') {
        try {
          ...
          setAttachments((prev) => [
            ...prev,
            { name: file.name, type: file.type, data: text, previewDataUrl, isRawText: true },
          ]);
        } catch {
        ...
        reader.onload = (ev) => {
          ...
          setAttachments((prev) => [
            ...prev,
            { name: file.name, type: file.type, data: base64.split(',')[1] },
          ]);
        };
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // Save user message (strip binary data before Firestore)
      try {
        await addDoc(collection(db, 'chats', currentChatId, 'messages'), {
          ...
          attachments: attachments.length > 0
            ? attachments.map(({ name, type, previewDataUrl }) => ({
                name,
                type,
        ...
        handleFirestoreError(err, OperationType.WRITE, `chats/${currentChatId}/messages`);
      }
      ...
      if (currentChatId) {
        try {
          await addDoc(collection(db, 'chats', currentChatId, 'messages'), {
            chatId: currentChatId,
            role: 'model',
            content: `⚠️ **Error generating summary:** ${error?.message ?? String(error)}\n\nPlease check the browser console for details.`,
            timestamp: serverTimestamp(),
          });
        } catch { /* ignore */ }
      }
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
        {messages.length === 0 && !isLoading && (
          <div className="h-full flex flex-col items-center justify-center text-center max-w-2xl mx-auto">
            ...
            <p className="text-white/40 text-lg leading-relaxed">
              Upload long documents (PDFs, Images) or paste long texts. I'll provide precise,
              hierarchical summaries including visual analysis of graphs and illustrations.
            </p>
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            <div className={cn('flex-1 space-y-4', msg.role === 'user' ? 'text-right' : 'text-left')}>
              {msg.attachments && msg.attachments.length > 0 && (
                ...
                >
                  {msg.attachments.map((att, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-2 px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg text-xs text-white/60"
                    >
                      {att.previewDataUrl ? (
                        <img
      ...
      <div className="p-6 border-t border-white/10 bg-[#0a0a0a]/80 backdrop-blur-xl">
        <div className="max-w-4xl mx-auto space-y-4">
            ...
            {attachments.length > 0 && (
              ...
              >
                {attachments.map((att, i) => (
                  <div
                    key={i}
                    className="group relative flex items-center gap-2 px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-sm text-white/80"
                  >
                    {att.previewDataUrl ? (
                      <img
                        src={att.previewDataUrl}
          ...
          <p className="text-[10px] text-center text-white/20 uppercase tracking-widest">
            Gemini 2.5 Flash • Hierarchical Summarization • Multimodal RAG
          </p>
        </div>
      </div>
    </div>
